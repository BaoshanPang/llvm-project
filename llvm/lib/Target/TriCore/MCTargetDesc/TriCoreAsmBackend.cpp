...
public:
    TriCoreAsmBackend(const Target &T) : MCAsmBackend(support::little) {}
...
public:
  ...
  ELFTriCoreAsmBackend(const Target &T, Triple::OSType OSType)
      : TriCoreAsmBackend(T), OSType(OSType) {}
...
MCAsmBackend *llvm::createTriCoreAsmBackend(const Target &T,
  ...
  return new ELFTriCoreAsmBackend(T, STI.getTargetTriple().getOS());
}
